...
class Node(ABC):
    ...
    async def async_execute(self, input:Any, adversarial_answers:Dict[str,str]={}, **kwargs):
        ...
        if getattr(self, "role", None) in ADV_ROLES and self.id in adversarial_answers:
            # Get the fixed adversarial answer
            fixed_answer = adversarial_answers[self.id]
            if fixed_answer not in ["A", "B", "C", "D"]:
                pdb.set_trace()
    ...
    @abstractmethod
    async def _async_execute(self, input:List[Any], comm_info:Dict[str,Dict], **kwargs):
        """ To be overriden by the descendant class """
        """ Use the processed input to get the result """

    @abstractmethod
    def _process_inputs(self, raw_inputs:List[Any], comm_info:Dict[str,Dict], **kwargs)->List[Any]:
        """ To be overriden by the descendant class """
